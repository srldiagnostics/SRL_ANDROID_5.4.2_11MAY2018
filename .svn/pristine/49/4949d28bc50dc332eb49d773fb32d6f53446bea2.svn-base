...
public class Util
{
	...
	public static String getStoredlointype(Context context)
	{
		String login = "";
		SharedPreferences pref = context.getApplicationContext().getSharedPreferences(Constants.login_credentials, 0); // 0 - for private mode
		SharedPreferences.Editor editor = pref.edit();
		login = pref.getString(Constants.loggedin_otp, null);
	...
	public static void clearRecomondationCart(Context context){
		DatabaseHelper db = new DatabaseHelper(context);
		List<BookTestORPackagesData> bookTestORPackagesDatas=null;
		try {
			if (db != null) {
		...
		if (bookTestORPackagesDatas != null) {
			for (BookTestORPackagesData bookTestORPackagesData : bookTestORPackagesDatas) {
				if (bookTestORPackagesData.isCart()) {
					if (!bookTestORPackagesData.getBook_pkg().equalsIgnoreCase("R")) {
						//book a test & packages
						//  continie with book a test and package
					} else {
						//Recommondation
						//clear the recommondation cart data

						final android.app.AlertDialog.Builder builder = new android.app.AlertDialog.Builder(context,
								AlertDialog.THEME_HOLO_LIGHT);
						//builder.setTitle();
						builder.setMessage("Already Added Recommoded packages.\nDo you want to clear cart and Continue?");
	...
	public static final int[] CERTIFICATES = {
			R.raw.srl_cer_der,   // you can put several certificates
	};
	public static ArrayList<SslCertificate> certificates;

	public static void loadSSLCertificates(Context context) {
		try {
			...
			for (int rawId : CERTIFICATES) {
				...
				try {
					...
					if (certificate instanceof X509Certificate) {
						...
						android.util.Log.w("HealthSurvey", "Wrong Certificate format: " + rawId);
					}
				} catch (CertificateException exception) {
					android.util.Log.w("HealthSurvey", "Cannot read certificate: " + rawId);
				} finally {
					try {
