...
public class OrderConfirmationActivity extends MenuBaseActivity implements View.OnClickListener {
    ...
    TextView orderNoTVID, thankyou, order,lblsurvey;
    Context context;
    DatabaseHelper db;
    ProductHeaderDatabase productHeaderDB;
    TextView track_my_order, datecommet, btnHome,takesurvey;
...
/////
        ...
        takesurvey=(TextView)findViewById(R.id.takesurvey);
        lblsurvey =(TextView)findViewById(R.id.lblsurvey);
        takesurvey.setOnClickListener(this);
        if(Constants.issrlRecommondation)
        {
            takesurvey.setVisibility(View.VISIBLE);
            btnHome.setText("Skip Survey 2");
            lblsurvey.setVisibility(View.VISIBLE);
        }
        else
        {
            takesurvey.setVisibility(View.GONE);
            btnHome.setText("  Home  ");
            lblsurvey.setVisibility(View.GONE);
        }
        ...
        eventValue.put(AFInAppEventParameterName.COUPON_CODE, promocode);
        eventValue.put(AFInAppEventType.ORDER_ID, orderId);
            ...
            case R.id.takesurvey:
                if(Constants.issrlRecommondation)
                {
                    ...
                    if (!Dashboard.selectedfamilymember.equalsIgnoreCase("")) {
                        ptntcd=Dashboard.selectedfamilymember;
                    }
                    else
                    {
                        ptntcd=Util.getStoredUsername(context);
                    }
                    //Devlopment
                  //  String url=ApiConstants.SURVEY_URL+"/hra?patient_id="+ptntcd;

                    //production
                     String url= ApiConstants.SURVEY_URL+"/hra?patient_id="+ptntcd;
                  //  Survey 2: https://srlcare.srl.in/hra?patient_id=TESTF2704620
                    Intent labintent = new Intent(context, HealthSurveySecond_activity.class);
