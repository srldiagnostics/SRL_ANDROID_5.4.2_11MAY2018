...
public class MyCartActivity extends MenuBaseActivity implements View.OnClickListener, BookATestAdapter.NotifyActivity
{
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		ly_promocode=(LinearLayout)findViewById(R.id.ly_promocode);
		...
		if(Constants.issrlRecommondation){
			...
			ly_promocode.setVisibility(View.GONE);
		}
		else if (!getIntent().hasExtra(Constants.REPEAT_ORDER))
		{
...
if(Constants.issrlRecommondation){
	for (BookTestORPackagesData bookdata : bookaTestDatas)
	{
		if (bookdata.isCart() && bookdata.getBook_pkg().equalsIgnoreCase("R") )
		{
			mBookaTestDatas.add(bookdata);
		}
	}
