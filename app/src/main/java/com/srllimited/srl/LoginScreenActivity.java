...
public class LoginScreenActivity extends MenuBaseActivity implements View.OnClickListener, RadioGroup.OnCheckedChangeListener, RestApiCallUtil.VolleyCallback {
    ...
    public static String commimgfromsurvey = "";
            ...
            switch (request.getReferralCode()) {
                ...
                case LOGIN: {
                    if (serverResponseData != null /*&& serverResponse.equalsIgnoreCase("Query Successful")*/) {
                        ...
                        editor.putString(Constants.loggedin_otp, "F");
                        editor.commit();
                        SharedPrefsUtil.setStringPreference(context, Constants.constantUsername, userid_edittext.getText().toString());
...
/*
                ...
                case Family_Member_Mapping:
                    if (serverResponseData.getMsg().equalsIgnoreCase("Query Successful")) {
                        ...
                        editor.putString(Constants.rememberme, remember_me.isChecked() + "");
                        editor.putString(Constants.loggedin_otp, "T");
                        editor.commit();
                        SharedPrefsUtil.setStringPreference(context, Constants.constantUsername, userid_edittext.getText().toString());
                        appDb = new AppDataBaseHelper(getApplicationContext());
                        orderDB = new OrderDatabase(getApplicationContext());
                    ...
                    if (serverResponseData.getMsg().equalsIgnoreCase("Query Successful")) {
                        ...
                        for (int i = 0; i < jArray.length(); i++) {
                            try {
                                ...
                                editor.putString(Constants.loggedin_otp, "T");
                                editor.commit();
                                SharedPrefsUtil.setStringPreference(context, Constants.constantUsername, jArray.getJSONObject(i).getString(Constants.ptnt_cd));
                                appDb = new AppDataBaseHelper(getApplicationContext());
                                orderDB = new OrderDatabase(getApplicationContext());
                            ...
                            for (int i = 0; i < jArray.length(); i++) {
                                try {
                                    ...
                                    if (jArray != null /*&& serverResponse.equalsIgnoreCase("Query Successful")*/) {
                                        ...
                                        sendRequest(ApiRequestHelper.Family_Member_Mapping(context, jArray.getJSONObject(i).getString(Constants.ptnt_cd), familymembers, "Android", Constants.devicetobepassed, jArray.getJSONObject(i).getString(Constants.mobile_no)));
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (commimgfromsurvey.equalsIgnoreCase("yes")) {
            login_as_guest_btn.setEnabled(false);
            login_as_guest_btn.setBackgroundColor(Color.parseColor("#ABABB2"));
            commimgfromsurvey = "";
        }
        ...
        logout_text.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (logout_text.getText().toString().equalsIgnoreCase("Login")) {
                    ...
                    if (!userid_edittext.getText().toString().trim().isEmpty() && !pwd_edittext.getText().toString().trim().isEmpty()) {
                    ...
                    android.app.AlertDialog.Builder builder = new android.app.AlertDialog.Builder(LoginScreenActivity.this, android.app.AlertDialog.THEME_HOLO_LIGHT);
                    builder.setMessage("Are you sure you want to logout?").setCancelable(false).setPositiveButton("No", new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            ...
                            Util.killLogin();
                            Intent aboutintent = new Intent(LoginScreenActivity.this, LoginScreenActivity.class);
        ...
        mOtpReader = new OTPReader(this, sPATTERN, new OTPReader.OnOTPListener() {
            ...
            public void onProcess() {
                ...
                if (RestApiCallUtil.mProgressDialog != null) {
                    RestApiCallUtil.mProgressDialog.setButton(DialogInterface.BUTTON_NEGATIVE, "Cancel", new DialogInterface.OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int which) {
                            mOtpReader.stop();
                            dialog.dismiss();
    ...
    private void VerifyOTP() {
        ...
        btnoptVerify.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                // Close dialog
                if (!(TextUtils.isEmpty(edtotp.getText().toString())) && TextUtils.isDigitsOnly(edtotp.getText().toString()))
                    sendRequest(ApiRequestHelper.verifyOTPLogin(context, mobile_edittext.getText().toString().trim(), edtotp.getText().toString().trim(), "Android", Constants.devicetobepassed));
            }
        });
    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.login_as_guest_btn:
                ...
                navigationcount = SharedPrefsUtil.getIntegerPreference(LoginScreenActivity.this, Constants.sharedPreferenceSelectedLoginActivity, 0);
    ...
    private boolean validateSelectedRadioEmail() {
        ...
        if (radio_email_edittext.getVisibility() == View.VISIBLE) {
            if (radio_email_edittext.getText().toString().isEmpty()) {
                ...
                if (radio_email_edittext.getText().toString().toLowerCase().trim().equalsIgnoreCase(fetchedEmailId.toLowerCase().trim())) {
                    isSelectedRadioEditEntered = true;
                } else {
        ...
        if (radio_mobileoremail_edittext.getVisibility() == View.VISIBLE) {
            if (radio_mobileoremail_edittext.getText().toString().isEmpty()) {
                ...
                if (radio_mobileoremail_edittext.getText().toString().toLowerCase().trim().equalsIgnoreCase(fetchedEmailId.toLowerCase().trim()) || radio_mobileoremail_edittext.getText().toString().equalsIgnoreCase(fetchedMobileNo)) {
                    isSelectedRadioEditEntered = true;
                } else {
    ...
    @Override
    public void onSuccessResponse(ApiRequestReferralCode referralCode, String serverResponse, HashMap<String, String> fetchValue) {
        ...
        switch (referralCode) {
            ...
            case LOGIN:
                if (serverResponse != null && serverResponse.equalsIgnoreCase("Query Successful")) {
                    ...
                    editor.commit();
                    SharedPrefsUtil.setStringPreference(context, Constants.constantUsername, userid_edittext.getText().toString());
                    SharedPrefsUtil.setStringPreference(context, Constants.loggedin_mobile, fetchValue.get(Constants.jsonFieldMOBILE));
                    SharedPrefsUtil.setStringPreference(context, Constants.loggedin_emailid, fetchValue.get(Constants.jsonFieldEmail));
                    ...
                    try {
                        ...
                        if (userd != null && !userd.equalsIgnoreCase("null")) {
                            int navigationcount = SharedPrefsUtil.getIntegerPreference(LoginScreenActivity.this, Constants.sharedPreferenceSelectedLoginActivity, 0);
                            navigateToActivity();
                        } else {
                            sendRequest(ApiRequestHelper.getUserDetails(context, Util.getStoredUsername(context)));
            ...
            case USER:
                ...
                if (serverResponse != null && !serverResponse.isEmpty() && serverResponse.equalsIgnoreCase("Query Successful") && view != null) {
            ...
            case FORGOT_PWD:
                ...
                if (serverResponse != null && serverResponse.split("--")[0].equalsIgnoreCase("Query Successful") && serverResponse.split("--")[1].equalsIgnoreCase("true")) {
                    if (alertDialog != null) {
                        alertDialog.dismiss();
                    }
                    Toast.makeText(context, "Your password has been sent to your email/mobile no", Toast.LENGTH_SHORT).show();
                } else if (serverResponse != null && serverResponse.split("--")[0].equalsIgnoreCase("Query Successful") && serverResponse.split("--")[1].equalsIgnoreCase("INVALID")) {
                    if (alertDialog != null) {
                        alertDialog.dismiss();
                    }
    ...
    private void navigateToActivity() {
        ...
        if (Validate.notEmpty(sharedpin)) {
            ...
            switch (navigationcount) {
                ...
                case Constants.m_recommondedpck:
                    Constants.issrlRecommondation = true;
                    Constants.isrecomm_comefrommenu = true;
                    Dashboard.selectedfamilymember = "";
                    Constants.isPackage = false;
                    Util.killBook();
                    Intent i = new Intent(LoginScreenActivity.this, BookATestActivity.class);
                    startActivity(i);
                    finish();
                    break;
