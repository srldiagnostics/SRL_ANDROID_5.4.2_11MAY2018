			...
			for (int i = 0; i < jArray.length(); i++)
			{
				try
				{
					...
					if (Validate.notEmpty(bannerFlg) && bannerFlg.equalsIgnoreCase("Y"))
					{
						...
						{
							Log.e("counti", i + "");
						//	SharedPrefsUtil.getStringPreference(mContext, "carouselimg" + i + "");

                          /*  SharedPrefsUtil.setIntegerPreference(mContext, "carouselcount", 0);
                            SharedPrefsUtil.setStringPreference(mContext, "carouselimg" + count + "", "");
                            SharedPrefsUtil.setLongPreference(mContext, imageUrl + "", 0);*/
                            ////
